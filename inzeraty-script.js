...
function renderAd(doc, ad, canEdit) {
  ...
  div.innerHTML = `
    ...
    ${canEdit ? `<button data-id="${doc.id}" class="edit-btn">Upravit</button>
...
function loadAds() {
  ...
  db.collection("inzeraty").get().then(snapshot => {
    snapshot.forEach(doc => {
      const ad = doc.data();
      const canEdit = currentUser && ad.uid === currentUser.uid;
      renderAd(doc, ad, canEdit);
    });
  });
}
...
adForm.onsubmit = (e) => {
  e.preventDefault();
  if (!currentUser) return alert("Musíte být přihlášeni pro přidání inzerátu.");
  ...
  const userInfo = {
    author: currentUser.email || "uživatel",
    uid: currentUser.uid
  };
  ...
  if (imageFile && imageFile.size > 0) {
    ...
    reader.onload = () => {
      ...
      saveAd({ ...data, ...userInfo });
    };
    reader.readAsDataURL(imageFile);
  } else {
    saveAd({ ...data, image: "", ...userInfo });
  }
};
...
auth.onAuthStateChanged(user => {
  ...
  loadAds();
  if (!user) {
    document.getElementById('adFormSection').innerHTML = '<p>Pro přidávání inzerátů se přihlašte.</p>';
  }
});